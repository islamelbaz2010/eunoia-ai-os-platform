-- Eunoia AI OS — Sprint 4: Enterprise Multi-Tenant Core (FIXED)
-- Replaces 0009_enterprise_multitenant.sql — do NOT apply both.
--
-- FIXES over the original:
--   1. CREATE TYPE IF NOT EXISTS is not valid PostgreSQL syntax.
--      Replaced with DO $$ BEGIN ... EXCEPTION WHEN duplicate_object THEN NULL; END $$
--   2. set_updated_at() trigger function was referenced but never defined.
--      Added CREATE OR REPLACE FUNCTION public.set_updated_at().
--   3. EXECUTE PROCEDURE deprecated in PG11+; replaced with EXECUTE FUNCTION.
--   4. Four CREATE POLICY statements lacked DROP POLICY IF EXISTS — added.
--   5. All ALTER TYPE ADD VALUE already have IF NOT EXISTS (safe to re-run).
--   6. All CREATE TABLE use IF NOT EXISTS (safe to re-run).
--   7. All CREATE FUNCTION use CREATE OR REPLACE (safe to re-run).
--   8. All INSERT seeds use ON CONFLICT DO NOTHING (safe to re-run).
--   9. All GRANTs are idempotent in PostgreSQL (safe to re-run).
--  10. All ALTER TABLE ... ADD COLUMN use IF NOT EXISTS (safe to re-run).
--
-- NOTES:
--   ALTER TYPE ... ADD VALUE is non-transactional. If run in a client that
--   wraps statements in BEGIN/COMMIT (e.g., psql with autocommit off), these
--   lines must be run separately first. The Supabase SQL Editor handles them
--   correctly when the file is pasted and run in one go.
--
-- Apply: paste entire file into
...
-- IF NOT EXISTS on ADD VALUE is supported in PostgreSQL 9.3+.
-- These are idempotent and can be re-run safely.
-- Each ADD VALUE commits immediately outside the transaction — this is expected
-- behavior and is safe in the Supabase SQL Editor.
-- =============================================================================
...
-- CREATE TYPE IF NOT EXISTS is NOT valid PostgreSQL syntax for enum types.
-- The correct pattern is a DO block with a duplicate_object exception handler.
-- =============================================================================
...
-- The set_updated_at() function is not a Supabase built-in — define it here.
...
-- DROP TRIGGER IF EXISTS + CREATE TRIGGER is the idempotent trigger pattern.
-- EXECUTE FUNCTION is the preferred syntax (EXECUTE PROCEDURE is deprecated in PG11+).
...
-- Convenience boolean check for single-permission RLS queries
...
-- All use CREATE OR REPLACE — idempotent.
-- =============================================================================
...
-- DROP POLICY IF EXISTS makes recreation idempotent.
-- =============================================================================
...
-- =============================================================================
-- PART 14: GRANTS
-- GRANTs are idempotent in PostgreSQL — safe to re-run.
-- =============================================================================
