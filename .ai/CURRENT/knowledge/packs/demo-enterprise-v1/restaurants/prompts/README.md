...
**Department**: Prompts (`prompts`)
**Schema Type**: `prompt.schema.json`

## Purpose

AI prompt structures, system instructions, and prompt engineering guidelines
...
| File | Purpose |
|------|---------|
| `schema.json` | JSON Schema for this department — extends `prompt.schema.json` |
...
The canonical schema for this department is `prompt.schema.json`, located at:
```
knowledge/schemas/prompt.schema.json
```
...
- `industry` — Must be `restaurants`
- `department` — Must be `prompts`
