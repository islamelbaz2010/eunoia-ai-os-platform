 ...
 *   1. Required fields — are all required fields present and non-null?
 *   2. Type checks — do field values match declared JSON Schema types?
 *   3. Enum checks — do enum fields contain valid values?
 *   4. Pattern checks — do regex-constrained fields match their patterns?
 *   5. Business rules — domain-specific checks (industry match, no placeholders, etc.)
 *   6. Metadata checks — language code, country code, version format, UUID format
 *   7. Checksum — if present, must follow sha256:hex format
 */
...
export function validate(
  ...
  if (doc.last_updated !== undefined && !ISO8601.test(String(doc.last_updated))) {
    err('FMT-005', 'last_updated', 'last_updated must be an ISO 8601 UTC timestamp', doc.last_updated, '2026-07-21T12:00:00.000Z');
  }
