...
import type { CanonicalSchema, SchemaType } from './types';

const cache = new Map<SchemaType, CanonicalSchema>();

export const SCHEMA_FILES: Record<SchemaType, string> = {
...
/** Resolve the absolute path to a schema file given the schemas root directory. */
function schemaPath(schemasRoot: string, schemaType: SchemaType): string {
  ...
  return path.join(schemasRoot, filename);
}
...
export async function loadAllSchemas(schemasRoot: string): Promise<number> {
  const types = Object.keys(SCHEMA_FILES) as SchemaType[];
  await Promise.all(types.map((t) => loadSchema(t, schemasRoot)));
  return types.length;
}
